<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<%@ page import="controller.*" %>
<%@ page import= "javax.swing.JOptionPane" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<link rel="stylesheet"
	href="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/5.0.0-beta3/css/bootstrap.min.css"
	integrity="sha512-N415hCJJdJx+1UBfULt+i+ihvOn42V/kOjOpp1UTh4CZ70Hx5bDlKryWaqEKfY/8EYOu/C2MuyaluJryK1Lb5Q=="
	crossorigin="anonymous" />
<title>Security Questions</title>
</head>
<style>


*::-webkit-scrollbar {
	width: 12px;
}

*::-webkit-scrollbar-track {
	background: #5c001f;
}

*::-webkit-scrollbar-thumb {
	background-color: #f8a517;
	border-radius: 20px;
	border: 3px #f8a517;
}

input {
	display: block;
	width: 100%;
	margin: 10px 0;
	padding: 10px;
	border-radius: 10px;
	border: 1px solid #eee;
	transition: .3s border-color;
}
</style>
<body>


	<div class="bg-image"
		style="background-image: url('https://i.ibb.co/9nfv7Dm/bg.png'); background-repeat: no-repeat; background-size: cover; height: 100vh">

		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
    <a class="navbar-brand" href="#"><img src="https://upload.wikimedia.org/wikipedia/commons/c/cb/UTM-LOGO-FULL.png" alt="" width="227" height="65"></a>
				<button class="navbar-toggler" type="button"
					data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent"
					aria-controls="navbarSupportedContent" aria-expanded="false"
					aria-label="Toggle navigation">
					<span class="navbar-toggler-icon"></span>
				</button>
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav ms-auto mb-2 mb-lg-0">

						<li class="nav-item"><div class="p-3"></div></li>

					</ul>
				</div>
			</div>
		</nav>

		<div class="container mt-5 mx-auto w-50">
			<div class="card shadow">
				<div class="card-header text-center bg-white border-0">
					<h1>Security Questions</h1>
				</div>
				<form action="/MyLibrary/updatePasswordController" method="POST">
					<div class="card-body ">
<%
   String email = request.getParameter("email");
updatePasswordController.getemail(email);
   if(updatePasswordController.getemail(email))
   { 
       JOptionPane.showMessageDialog(null, "Please enter an email that exists in database!");                  
	   response.sendRedirect("LoginStudent.jsp");
   }
   %>
					
						<h5 class="card-title">What is your favorite book?</h5>
						<p class="card-text">
							<input type="text" name="check1" placeholder="Favorite Book"
								required>
						</p>
						<h5 class="card-title">What is your favorite book category?</h5>
						<p class="card-text">
							<input type="text" name="check2" placeholder="Favorite Category"
								required>
						</p>
						<h5 class="card-title">Who is your favorite author</h5>
						<p class="card-text">
							<input type="text" name="check3" placeholder="Favorite Author"
								required>
						</p>
						<h5 class="card-title">New Password</h5>
						<p class="card-text">
							<input type="password" name="pass" placeholder="New Password" required>
						</p>
						<h5 class="card-title">Confirm Password</h5>
						<p class="card-text">
							<input type="password" name="cpass" placeholder="Confirm Password" required>
						</p>
					</div>
					<div class="card-footer text-center bg-white border-0">
					  <input type="hidden" name="email" value="<%=email%>">
						<input type="submit" class="btn btn-warning btn-lg" name="check" id="register"><br>
						<br>
					</div>
				</form>
			</div>
		</div>
	</div>
</body>
<script
	src="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/5.0.0-beta3/js/bootstrap.min.js"
	integrity="sha512-mp3VeMpuFKbgxm/XMUU4QQUcJX4AZfV5esgX72JQr7H7zWusV6lLP1S78wZnX2z9dwvywil1VHkHZAqfGOW7Nw=="
	crossorigin="anonymous"></script>
</html>